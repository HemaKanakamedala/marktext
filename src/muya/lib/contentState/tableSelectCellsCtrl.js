...
const tableSelectCellsCtrl = ContentState => {
  ...
  ContentState.prototype.deleteSelectedTableCells = function (isCut = false) {
    const { tableId, cells, row: rowsSel } = this.selectedTableCells
    ...
    const isOneColumnSelected = rows.size === +row + 1
    const isOneRowSelected = rowsSel === cells.length / (column + 1)
    ...
    if (hasContent) {
      ...
      if (isOneColumnSelected) {
        ...
        const colsDel = cellLength / (row + 1)
        for (let i = 0; i < colsDel; i++) {
        ...
        const rowsDel = cellLength / (column + 1)
        for (let i = 0; i < rowsDel; i++) {
          const cellKey = cells[i + column].key
          const cellBlock = this.getBlock(cellKey)
          const cellContentKey = cellBlock.children[0].key
          this.editTable({
            location: 'current',
